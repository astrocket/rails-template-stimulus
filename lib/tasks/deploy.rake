...
namespace :deploy do
  ...
  namespace :production do
    ...
    task :set_maseter_key => :environment do
      sh("kubectl create secret generic rails-template-stimulus-secrets --from-file=rails-template-stimulus-master-key=config/master.key")
    end

    namespace :ingress do
      task :up => :environment do
        sh("kubectl apply -f k8s/project/rails-template-stimulus-nginx-conf.yaml")
        sh("kubectl apply -f k8s/service.yaml")
        sh("kubectl apply -f k8s/ingress.yaml")
      end

      task :down => :environment do
        sh("kubectl delete secret rails-template-stimulus-secrets")
        sh("kubectl delete -f k8s/project/rails-template-stimulus-nginx-conf.yaml")
  ...
  namespace :logs do
    task :demo => :environment do
      log_containers("rails-template-stimulus-demo-web")
    end

    task :web => :environment do
      log_containers("rails-template-stimulus-web")
    end

    task :sidekiq => :environment do
      log_containers("rails-template-stimulus-sidekiq")
    end
  end
end