...
```bash
kubectl apply -f k8s/cluster/doks-metrics-server.yaml
```
...
```bash
kubectl describe certificate astemplate-demo-tls
...
Events:
  ...
  Normal  Requested     5m   cert-manager  Created new CertificateRequest resource "astemplate-demo-tls-1514794236"
...
* `k8s/project/astemplate-nginx-conf.yaml` creates application level nginx's(* is different from ingress-nginx*) config-map, where we serve static files, redirect app traffic to puma. (It will be used when deploying k8s/app.yaml)

```bash
kubectl create secret generic astemplate-secrets --from-file=astemplate-master-key=config/master.key # push master.key to k8s secret
...
kubectl apply -f k8s/project/astemplate-nginx-conf.yaml # for web deployment's nginx
...
# astemplate-demo-web-f45fbdf9-7wsqn   2/2     Running   0          18s
# astemplate-demo-web-f45fbdf9-fsxz8   2/2     Running   0          18s
# astemplate-demo-web-f45fbdf9-qhms6   2/2     Running   0          18s
# astemplate-demo-web-f45fbdf9-vqfmj   2/2     Running   0          18s

# for single pod (tail 5 lines)
kubectl logs -f --tail=5 astemplate-demo-web-f45fbdf9-7wsqn -c app
kubectl logs -f --tail=5 astemplate-demo-web-f45fbdf9-7wsqn -c nginx

# for all pods
kubectl logs -f --tail=5 --selector app=astemplate-demo-web -c app
kubectl logs -f --tail=5 --selector app=astemplate-demo-web -c nginx
```
...
```bash
kubectl create secret generic astemplate-secrets --from-file=astemplate-master-key=config/master.key
```
...
```yaml
kubectl get secret astemplate-secrets -o jsonpath="{.data.astemplate-master-key}" | base64 --decode
```

delete secrets

```bash
kb delete secret astemplate-secrets
```
...
You can customize application nginx through [config-map](k8s/project/astemplate-nginx-conf.yaml) as usual.
